...
import org.teng.domain.Employee;
...
public class EmployeeController {
	...
	@RequestMapping(value="/employee/selectEmployee")
	 public String selectEmployee(Integer pageIndex,
		...
		// 查询职位信息，用于模糊查询
		List<Job> jobs = tengService.findAllJob();
		// 查询部门信息 ，用于模糊查询
		List<Dept> depts = tengService.findAllDept();
		// 查询员工信息    
		List<Employee> employees = tengService.findEmployee(employee,pageModel);
		// 设置Model数据
	...
	@RequestMapping(value="/employee/addEmployee")
	 public ModelAndView addEmployee(
		...
		if(flag.equals("1")){
			...
			this.genericAssociation(job_id, dept_id, employee);
			// 添加操作
			tengService.addEmployee(employee);
			// 设置客户端跳转到查询请求
	...
	@RequestMapping(value="/employee/removeEmployee")
	 public ModelAndView removeEmployee(String ids,ModelAndView mv){
		...
		for(String id : idArray){
			// 根据id删除员工
			tengService.removeEmployeeById(Integer.parseInt(id));
		}
		// 设置客户端跳转到查询请求
...
//		mv.setViewName("forward:/employee/selectEmployee");
	...
	@RequestMapping(value="/employee/updateEmployee")
	 public ModelAndView updateEmployee(
		...
		if(flag.equals("1")){
			// 根据id查询员工
			Employee target = tengService.findEmployeeById(employee.getId());
			// 需要查询职位信息 
			List<Job> jobs = tengService.findAllJob();
			// 需要查询部门信息 
			List<Dept> depts = tengService.findAllDept();
			...
			// 执行修改操作
			tengService.modifyEmployee(employee);
			// 设置客户端跳转到查询请求
			mv.setViewName("redirect:/employee/selectEmployee");
		}
		// 返回

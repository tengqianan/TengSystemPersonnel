...
public class DocumentController {
	 ...
	 * */
	@Autowired
	@Qualifier("tengService")
	private TengService tengService;
	...
	@RequestMapping(value="/document/selectDocument")
	 public String selectDocument(
		...
		/** 查询用户信息     */
		List<Document> documents = tengService.findDocument(document, pageModel);
	...
	@RequestMapping(value="/document/addDocument")
	 public ModelAndView addDocument(
		...
		if(flag.equals("1")){
			...
			// 设置关联的User对象
			User user = (User) session.getAttribute(tengConstants.USER_SESSION);
			document.setUser(user);
			// 插入数据库
			tengService.addDocument(document);
			// 返回
	...
	@RequestMapping(value="/document/removeDocument")
	 public ModelAndView removeDocument(String ids,ModelAndView mv){
		...
		for(String id : idArray){
			// 根据id删除文档
			tengService.removeDocumentById(Integer.parseInt(id));
		}
		// 设置客户端跳转到查询请求
		mv.setViewName("redirect:/document/selectDocument");
		// 返回ModelAndView
	...
	@RequestMapping(value="/document/updateDocument")
	 public ModelAndView updateDocument(
		...
		if(flag.equals("1")){
			// 根据id查询文档
			Document target = tengService.findDocumentById(document.getId());
			// 设置Model数据
			...
			// 执行修改操作
			tengService.modifyDocument(document);
			// 设置客户端跳转到查询请求
			mv.setViewName("redirect:/document/selectDocument");
		}
		// 返回
	...
	@RequestMapping(value="/document/downLoad")
	 public ResponseEntity<byte[]>  downLoad(
			 ...
			 HttpSession session) throws Exception{
		// 根据id查询文档
		Document target = tengService.findDocumentById(id);
